...
async function bcryptHash(value, saltRounds) {
  const bcryptedMaster = await bcrypt.hash(value, saltRounds);
  return bcryptedMaster;
}

async function bcryptHashCompare(password, passwordHash) {
  const bcryptMatchPassword = await bcrypt.compare(password, passwordHash);
  return bcryptMatchPassword;
}
...
exports.bcryptHashCompare = bcryptHashCompare;
